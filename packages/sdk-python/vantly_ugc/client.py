...
"""Sync + async client for vantly-ugc API v2."""
...
DEFAULT_BASE_URL = "https://api.vantly-ugc.com"


class VantlyUgcError(Exception):
...
class VantlyUgc:
    """Sync client for vantly-ugc video generation API.
    ...
    def __init__(
        ...
        from .v2 import VantlyUgcV2  # local import to avoid circular ref
        self.v2 = VantlyUgcV2(self._client)
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        if not resp.is_success:
            ...
            if isinstance(error, dict):
                raise VantlyUgcError(
                    resp.status_code,
                    error.get("code", "UNKNOWN"),
                    error.get("message", "Request failed"),
                    error.get("details"),
                )
            raise VantlyUgcError(resp.status_code, "UNKNOWN", str(data))
        return data
    ...
    def create_product_acting(
        ...
        **params: Any,
        ...
        raise VantlyUgcError(408, "TIMEOUT", f"Timed out. Job: {job['job_id']}")
    ...
    def create_video(self, timeout_seconds: int = 600, poll_interval: int = 5, **params: Any) -> dict:
        ...
        raise VantlyUgcError(408, "TIMEOUT", f"Timed out after {timeout_seconds}s. Job: {job['job_id']}")
    ...
    def create_character_video(
        ...
        if not character_sheet_url:
            raise VantlyUgcError(500, "GENERATION_FAILED", f"character sheet completed without URL: {sheet}")
        ...
        if not storyboard_url:
            raise VantlyUgcError(500, "GENERATION_FAILED", f"storyboard completed without URL: {sb}")
        ...
        if not video_url:
            raise VantlyUgcError(500, "GENERATION_FAILED", f"video completed without URL: {video}")
    ...
    def _poll_until_done(self, job_id: str, label: str, timeout_seconds: int, poll_interval: int) -> dict:
        ...
        while time.time() - start < timeout_seconds:
            ...
            time.sleep(poll_interval)
        raise VantlyUgcError(408, "TIMEOUT", f"{label} timed out. Job: {job_id}")

    def close(self) -> None:
        """Close the underlying HTTP client."""
        self._client.close()

    def __enter__(self) -> "VantlyUgc":
        return self
...
class AsyncVantlyUgc:
    """Async client for vantly-ugc video generation API."""
    ...
    def __init__(
        ...
        from .v2 import AsyncVantlyUgcV2  # local import to avoid circular ref
        self.v2 = AsyncVantlyUgcV2(self._client)
    ...
    async def create_show_your_app(
        ...
        while time.time() - start < timeout_seconds:
            ...
            if status["status"] == "failed":
                raise VantlyUgcError(500, "GENERATION_FAILED", status.get("error_message", "Failed"))
            await asyncio.sleep(poll_interval)
        raise VantlyUgcError(408, "TIMEOUT", f"Timed out. Job: {job['job_id']}")
    ...
    async def __aenter__(self) -> "AsyncVantlyUgc":
        return self
