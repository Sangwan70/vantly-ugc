...
/**
 * Admin-only "Share with users" popup for a single gallery item — search by
 * email/name, bulk-select recipients, push a snapshot of the item straight
 * into their own galleries (POST /api/admin/gallery/share). See
 * app/api/admin/gallery/share/route.ts and the gallery_shares migration for
 * why this is admin-only and denormalized.
 */
...
export function ShareModal({ item, onClose }: { item: ShareableItem; onClose: () => void }) {
  ...
  async function handleSend() {
    ...
    setError(null);
    try {
      const resp = await fetch('/api/admin/gallery/share', {
        method: 'POST',
        credentials: 'include',
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold" style={{ color: '#E9E9F0' }}>Share with users</h3>
