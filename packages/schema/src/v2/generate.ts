...
/**
 * `model: "auto"`, let vantly-ugc pick from the live catalog using the
 ...
 *
 * Note: until a `rate_run` / auto-judge scoring pipeline is wired up (a
 * separate, larger piece of upstream we have NOT ported — see the
 * upstream-comparison report), `stats` is always passed in empty from the
 * caller, so pickAuto degrades gracefully to "always the kind's default"
 * with an honest reason string. That is a safe, intentional subset: never
 * a silent no-op, never a crash, just no smart routing yet.
 */
...
export const GenerateImageSchema = z
  .object({
    ...
    model: liveModelField('image').describe('A live image model id from list_models, or "auto" to let vantly-ugc pick from recent results. Omit for the default (gpt-image-2.5).'),
...
const VideoBase = z
  .object({
    ...
    model: liveModelField('video').describe('A live video model id from list_models, or "auto" to let vantly-ugc pick from recent results. Omit for the default. Call list_models for what each model is good for, its modes, limits and price.'),
 ...
 * Pure. Never returns a non-live model. With an empty `stats` map (today's
 * default — see the note on V2_MODEL_AUTO above) this always falls through
 * to step 4.
 */
