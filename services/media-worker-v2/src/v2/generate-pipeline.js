 ...
 *   processGenerateImage  prompt (+refs) → gpt-image-2.5 (catalog default) → R2 png
 ...
 * request against the catalog (@vantly-ugc/schema/v2 V2_MODELS) and sent
 ...
 *
 * providerUsage (from the EvoLink task's `usage`) is returned so a caller
 * can record what the provider actually billed, but we do NOT yet write
 * it anywhere (no generation_quality table / auto-judge pipeline in this
 * repo — that is a separate, larger upstream feature we have not ported;
 * see the upstream-comparison report). It is carried through so that
 * follow-up work does not have to touch this file again to get at it.
 */
 ...
 * in @vantly-ugc/schema/v2 (the source of truth, from the EvoLink specs);
...
// @vantly-ugc/schema/v2 (generate.ts); a raw id passes through.
const VOICES = {
  ...
  if (broken.length) {
    const e = new Error(
      `reference image is corrupt or unreadable, so the render was not started: ${broken.join('; ')}. ` +
        'Re-upload the ORIGINAL file, which streams it whole; a truncated base64 upload produces exactly this.',
    );
...
export async function processGenerateVideo(params) {
  ...
  const { url: providerUrl, task } = await runGenerationDetailed(providerModel, body, { timeoutMs });
...
export async function processGenerateAudio(params) {
  ...
  try {
    // generateElevenLabsTTS writes a WAV; deliver mp3.
 ...
 * Used so api-v2 can bill reference clips at submit.
 */
