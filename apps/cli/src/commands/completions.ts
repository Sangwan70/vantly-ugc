...
/**
 * `vantly-ugc completions <shell>` command.
 *
 ...
 *
 * Usage:
 *   vantly-ugc completions bash >> ~/.bashrc
 *   vantly-ugc completions zsh  >> ~/.zshrc
 *   vantly-ugc completions fish > ~/.config/fish/completions/vantly-ugc.fish
 */
...
function generateBash(): string {
  ...
  return `# vantly-ugc bash completion
# Generated by: vantly-ugc completions bash
# Add to ~/.bashrc or ~/.bash_profile:
#   eval "$(vantly-ugc completions bash)"

_vantly_ugc_completions() {
...
complete -F _vantly_ugc_completions vantly-ugc
`;
}
...
function generateZsh(): string {
  ...
  return `#compdef vantly-ugc

# vantly-ugc zsh completion
# Generated by: vantly-ugc completions zsh
# Add to ~/.zshrc:
#   eval "$(vantly-ugc completions zsh)"
# Or save to a file in your $fpath:
#   vantly-ugc completions zsh > ~/.zsh/completions/_vantly-ugc
...
${cmdLines}
    ...
    case "$state" in
        command)
            _describe -t commands 'vantly-ugc command' commands
            ;;
        args)
            case "$words[1]" in
                completions)
...
function generateFish(): string {
  const cmdCompletions = COMMANDS.map(
    (cmd) => `complete -c vantly-ugc -n "__fish_use_subcommand" -a "${cmd}" -d "${cmd} command"`,
  ).join('\n');

  return `# vantly-ugc fish completion
# Generated by: vantly-ugc completions fish
# Save to: ~/.config/fish/completions/vantly-ugc.fish

# Disable file completions by default
complete -c vantly-ugc -f

# Global flags
complete -c vantly-ugc -l json -d "Output as JSON"
complete -c vantly-ugc -l quiet -d "Minimal output"
complete -c vantly-ugc -l no-color -d "Disable colored output"
complete -c vantly-ugc -l verbose -d "Show debug output"
complete -c vantly-ugc -l profile -d "Use a specific credential profile" -x
complete -c vantly-ugc -l help -d "Show help"
complete -c vantly-ugc -l version -d "Show version"

# Subcommands
${cmdCompletions}

# completions subcommand
complete -c vantly-ugc -n "__fish_seen_subcommand_from completions" -a "bash zsh fish" -d "Shell type"

# config subcommand
complete -c vantly-ugc -n "__fish_seen_subcommand_from config" -a "get set list reset export import" -d "Config action"

# credits subcommand
complete -c vantly-ugc -n "__fish_seen_subcommand_from credits" -a "estimate history topup" -d "Credits action"

# ugc subcommand (no positional completions, uses flags)

# actor subcommand
complete -c vantly-ugc -n "__fish_seen_subcommand_from actor" -a "list preview" -d "Actor action"

# persona subcommand
complete -c vantly-ugc -n "__fish_seen_subcommand_from persona" -a "list create delete" -d "Persona action"

# apikey subcommand
complete -c vantly-ugc -n "__fish_seen_subcommand_from apikey" -a "list create revoke" -d "API key action"

# profile subcommand
complete -c vantly-ugc -n "__fish_seen_subcommand_from profile" -a "list use delete export" -d "Profile action"

# alias subcommand
complete -c vantly-ugc -n "__fish_seen_subcommand_from alias" -a "set remove list" -d "Alias action"
`;
}
