...
class VantlyUgcV2:
    """
    Synchronous v2 surface. Bound to an VantlyUgc instance — users access it
    via `client.v2`. Each method is a thin wrapper over an api-v2 REST route
    declared in @vantly-ugc/schema/v2 → V2_GENERATORS.
    """
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        from ..client import VantlyUgcError  # type: ignore
        data: Any
        try:
            data = resp.json()
        except Exception:
...
class AsyncVantlyUgcV2:
    """
    Async variant of VantlyUgcV2. Bound to an AsyncVantlyUgc instance —
