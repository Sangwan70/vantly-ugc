...
// ── The catalog ───────────────────────────────────────────────────────────
//
// Credit prices for the live models are what a loose-surface
// generate_video/generate_image/generate_audio call bills today. This
// catalog is intentionally separate from V2_GENERATORS (packages/schema/
// src/v2/generators.ts), which prices our fixed skills (selfie, crazy
// look, podcast, storybook, ...) on its own flat per-second tables. The
// two are NOT required to match: a fixed skill's price is the cost of
// its whole recipe (portrait + sheet + wireframe + clip), while a catalog
// entry prices one raw provider call. Do not assume changing one changes
// the other.
