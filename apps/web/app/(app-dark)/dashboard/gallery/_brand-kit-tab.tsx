...
/**
 * Brand Kit tab of /dashboard/gallery — unchanged from the former standalone
 * /dashboard/brand-kit page (still read-only: extracted from the user's site,
 * with a re-extract button; no manual-edit fields yet), just without its own
 * page header since the shell now renders that.
 */
...
export function BrandKitTab() {
  ...
  return (
    <div>
      {loading ? (
        <div className="flex h-64 items-center justify-center rounded-3xl" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      {error ? (
        <div className="mt-6 rounded-2xl px-4 py-3 text-sm" style={{ border: '1px solid rgba(255,79,79,0.3)', backgroundColor: 'rgba(255,79,79,0.08)', color: '#FCA5A5' }}>
          {error}
        </div>
      ) : null}
    </div>
...
function BrandView({
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.4fr_1fr]">
      <div className="overflow-hidden rounded-3xl" style={{ border: '1px solid rgba(255,255,255,0.06)', backgroundColor: '#0B0C11' }}>
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="flex items-center gap-3">
            ...
            <a href={brand.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs transition-opacity hover:opacity-80" style={{ color: 'rgba(255,255,255,0.6)' }}>
              {hostname}
          ...
          >
            {reExtracting ? <Loader2 className="h-3 w-3 animate-spin" /> : <RefreshCw className="h-3 w-3" />}
            Re-extract
          </button>
        </div>
        <div className="max-h-[640px] overflow-y-auto" style={{ backgroundColor: '#FFFFFF' }}>
          {brand.screenshot ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={brand.screenshot} alt="Website screenshot" className="block w-full" />
          ) : (
      ...
      <div className="flex flex-col gap-4">
        <div className="rounded-3xl p-5" style={{ backgroundColor: '#191A22', border: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="flex items-start gap-4">
            {brand.logo && !/\.ico(\?|$)/i.test(brand.logo) ? (
              ...
              <img
                ...
                style={{ border: '1px solid rgba(255,255,255,0.08)', backgroundColor: '#FFFFFF', padding: 6 }}
              />
            ) : (
              <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl" style={{ border: '1px solid rgba(255,255,255,0.08)', backgroundColor: '#1F202A' }} aria-hidden>
                <Globe className="h-5 w-5" style={{ color: 'rgba(255,255,255,0.4)' }} />
              </span>
            )}
            <div className="flex min-w-0 flex-col gap-1">
              <p className="text-base font-semibold" style={{ color: '#E9E9F0', letterSpacing: '-0.01em' }}>{displayName}</p>
              <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>{hostname}</p>
            </div>
          </div>
        </div>

        <div className="rounded-3xl p-5" style={{ backgroundColor: '#191A22', border: '1px solid rgba(255,255,255,0.06)' }}>
          <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.18em]" style={{ color: 'rgba(255,255,255,0.5)' }}>Palette</p>
          {palette.length === 0 ? (
            <p className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>No palette extracted.</p>
          ) : (
            <div className="flex flex-wrap items-center gap-2">
              {palette.map((hex) => (
                <span
                  ...
                  title={hex}
                  style={{ backgroundColor: hex, color: 'rgba(255,255,255,0.85)', textShadow: '0 1px 0 rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.08)' }}
                >
                  {hex.replace('#', '')}
                </span>
              ))}
        ...
        {brand.title || brand.description ? (
          <div className="rounded-3xl p-5" style={{ backgroundColor: '#191A22', border: '1px solid rgba(255,255,255,0.06)' }}>
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.18em]" style={{ color: 'rgba(255,255,255,0.5)' }}>Messaging</p>
            {brand.title ? <p className="text-sm font-medium" style={{ color: '#E9E9F0' }}>{brand.title}</p> : null}
            {brand.description ? <p className="mt-2 text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.55)' }}>{brand.description}</p> : null}
          </div>
        ) : null}
      </div>
    </div>
...
function UrlForm({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-5 rounded-3xl py-16 px-6" style={{ border: '1px solid rgba(255,255,255,0.06)', backgroundColor: '#15161D' }}>
      <div className="flex flex-col items-center gap-2 text-center">
        <p className="text-base font-medium" style={{ color: '#E9E9F0' }}>No brand kit yet</p>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            color: '#0F1015',
            boxShadow: submitting ? 'none' : '0 8px 24px rgba(167,139,250,0.25), 0 0 0 1px rgba(255,255,255,0.6)',
          }}
        >
          {submitting ? (
            <>
